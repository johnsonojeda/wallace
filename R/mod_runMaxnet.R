...
runMaxnet_UI <- function(id) {
  ...
  tagList(
    ...
    tags$div(title='If checked, RM selection will be done via glmnet.',
             checkboxInput(ns('glmnetRM'), label = 'glmnet RM select')),
    ...
    #tags$div(title='Range of regularization multipliers to explore. Greater values of the regularization multiplier lead to increased penalty against overly complex and/or overfit models. A value of 0 results in no regularization.',
     #       sliderInput(ns("rms"), label = "", min = 0.5, max = 10, step=0.5, value = c(1, 2))),
...
runMaxnet_MOD <- function(input, output, session) {
  reactive({
    ...
    for(sp in spIn()) {  
      ...
      # FUNCTION CALL ####
      m.maxnet <- runMaxnet(spp[[sp]]$occs, 
                            spp[[sp]]$bg, 
      ...
      # METADATA ####
      spp[[sp]]$rmm$model$algorithm <- "Maxnet"
      spp[[sp]]$rmm$model$maxnet$featureSet <- input$fcs
      spp[[sp]]$rmm$model$maxnet$regularizationMultiplierSet <- input$rms
      spp[[sp]]$rmm$model$maxnet$regularizationRule <- paste("increment by", input$rmsStep)
      spp[[sp]]$rmm$model$maxnet$notes <- "dismo package implementation"
    }
    })
}

runMaxnet_INFO <- infoGenerator(modName = "Maxnet",
                               modAuts = "TBD",
                                pkgName = c("glmnet", "dismo"))

runMaxnet_TBL <- function(input, output, session) {
